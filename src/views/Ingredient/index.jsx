...
import IngredientManage from './IngredientManage'
import { Button } from '@mui/material'
import AddBoxOutlinedIcon from '@mui/icons-material/AddBoxOutlined'
import CircularProgress from '@mui/material/CircularProgress'
import CustomSearch from '@/components/CustomSearch'
import { useFetchIngredientsMutation, useChangeIngredientStatusMutation } from '@/store/apis/ingredientApis'
import { formikInitialValuesFunc, searchFormFunc, tableActionsFunc, tableColumnsFunc } from './indexObjects'
import { useParams } from 'react-router-dom'

const Ingredient = (props) => {
  const [fetchIngredients, results] = useFetchIngredientsMutation()
  const [changeStatus, statusResults] = useChangeIngredientStatusMutation()
  ...
  const fromComponent = (from) => {
    if (from === 'ingredientAction') {
      fetchIngredients(formik.values)
    }
  }
  ...
  return (
    <Layout>
      ...
      <Button variant='outlined' onClick={() => toggleFullScreenDialog(true, 'new', 0)}>
        <AddBoxOutlinedIcon /> <span className='ml-5'> Add New Ingredient </span>
      </Button>
      ...
      <DataTable
        ...
        paginationChange={fetchIngredients}
        isLoading={results.isLoading}
        isSuccess={results.isSuccess}
      />

      {manage.popup && (
        <IngredientManage data={manage} toggleFullScreenDialog={toggleFullScreenDialog} fromComponent={fromComponent} />
      )}

      <CustomDialog open={actionStatus} handleClose={() => setActionStatus(false)} title='Change ingredient status'>
